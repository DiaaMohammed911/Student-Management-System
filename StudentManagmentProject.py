...
Overview:
A professional desktop application for managing student accounts,
built with Python, Tkinter (GUI), SQLite (database), and Pillow (image processing).

This system demonstrates a complete student workflow:
- Student & Admin authentication
- Student registration & account management
- Password recovery via email (SMTP integration)
- Student ID card generator with images
- Admin dashboard with search, CRUD operations, announcements, and statistics

Key Features:
- Role-based access (Student vs Admin)
- Persistent storage using SQLite
- GUI built with Tkinter for ease of use
- Email notifications for recovery & announcements
- Image handling with Pillow to generate student cards
...
Purpose:
- Portfolio project showcasing OOP design, GUI development,
  database integration, and software engineering best practices.
- Demonstrates practical implementation of a real-world
  Student Management & Registration workflow.
"""
...
root = tk.Tk()
root.geometry('580x608')
root.title('Tkinter Hub (Student Management & Registration System)')
bg_color="#273b7a"
login_student_icon = tk.PhotoImage(file="C:/the rood/Students Management&Registration Project/login_student_img.png")
login_admin_icon = tk.PhotoImage(file="C:/the rood/Students Management&Registration Project/admin_img.png")
add_student_icon = tk.PhotoImage(file="C:/the rood/Students Management&Registration Project/add_student_img.png")
locked_icon = tk.PhotoImage(file="C:/the rood/Students Management&Registration Project/locked.png")
un_locked_icon = tk.PhotoImage(file="C:/the rood/Students Management&Registration Project/unlocked.png")
add_student_pic_icon=tk.PhotoImage(file="C:/the rood/Students Management&Registration Project/add_image.png")
...
def add_data(id_number,password,name,age,gender,phone_number,
             studentclass,email,pic_data):
    connection = sqlite3.connect('students_accounts.db')

    cursor=connection.cursor()

    cursor.execute(f"""
    INSERT INTO data VALUES('{id_number}',
    '{password}','{name}','{age}','{gender}','{phone_number}','{studentclass}'
    ,'{email}',?)
    """,[pic_data])
    connection.commit()
    connection.close()
...
def message_box(message):
    ...
    close_btn=tk.Button(message_box_fm,text='X',bd=0
                        ,font=('Bold',13),fg=bg_color
...
def student_card(student_pic_path,student_data):
    labels="""
Id Number:
Name:
Gender:
...
Email:     
"""
    student_card = Image.open("C:/the rood/Students Management&Registration Project/student_card_frame.png")
...
def Student_Card_Page(student_card_obj,bypass_login_page=False):
    ...
    def print_student_card():
        ...
        if path:
            ...
            student_card_obj.save(f'{path}/student_card.png')
            win32api.ShellExecute(0,'print',f'{path}/student_card.png',None
                                  ,'.',0)
    ...
    card_img=ImageTk.PhotoImage(file="C:/the rood/Students Management&Registration Project/student_card_frame.png")
...
def welcomePage():
    ...
    welcome_page_fm = tk.Frame(root, highlightbackground=bg_color, highlightthickness=2)
    heading_lb = tk.Label(
        welcome_page_fm,
    ...
    student_login_btn = tk.Button(welcome_page_fm, text='Login Student', bg=bg_color, fg='white',
                                font=('bold', 15), bd=0
                                ,command=forward_to_student_login_page)
    student_login_btn.place(x=120, y=125, width=200)

    student_login_icon = tk.Button(welcome_page_fm, image=login_student_icon, bd=0,
                                   command=forward_to_student_login_page)
    student_login_icon.place(x=60, y=100)
...
def forget_password_page():
    ...
    def recover_password():
        if check_id_already_exists(id_number=student_id_et.get()):
            print('correct id')
            connection = sqlite3.connect('students_accounts.db')
            ...
            cursor.execute(f"""
            SELECT password FROM data where id_number == '{student_id_et.get()}'
        """)
            connection.commit()
            
            recover_password=cursor.fetchall()[0][0]
            print('recover password:',recover_password)

            cursor.execute(f"""
            SELECT email FROM data where id_number == '{student_id_et.get()}'
        """)
            connection.commit()
            student_email=cursor.fetchall()[0][0]
            print('email addres :',student_email)
            connection.close()
...
Do You Want To Confirm?""")
    ...
    forget_password_page_fm = tk.Frame(root,highlightbackground=bg_color
                                       ,highlightthickness=3)
    heading_lb=tk.Label(forget_password_page_fm,text='⚠️ Forgetting Password',
    ...
    student_id_lb.config(
    font=("Bold", 13)
)
    student_id_lb.place(x=70, y=40)

    student_id_et = tk.Entry(forget_password_page_fm,
    font=("Bold", 15), justify=tk.CENTER
    )
    student_id_et.place(x=70, y=70, width=180)
...
def fetch_student(query):
    ...
    connection.commit()
    responde=cursor.fetchall()
    connection.close()

    return responde

def student_dashboard(student_id):
    get_student_details=fetch_student(f"""
    SELECT name, age, gender, class, phone_number, email FROM data WHERE id_number == '{student_id}' 
""")
    get_student_pic=fetch_student(f"""
...
""")
    ...
    def switch(indecator,page):
        home_btn_indecator.config(bg='#c3c3c3')
        student_card_btn_indecator.config(bg='#c3c3c3')
        security_btn_indecator.config(bg='#c3c3c3')
        edit_data_btn_indecator.config(bg='#c3c3c3')
        delete_btn_indecator.config(bg='#c3c3c3')
        for child in pages_fm.winfo_children():
            child.destroy()
            root.update()
        page()
        
        indecator.config(bg=bg_color)

    dashboard_frame = tk.Frame(root, highlightbackground=bg_color,
                           highlightthickness=3)
    options_fm = tk.Frame(dashboard_frame, highlightbackground=bg_color,
                      highlightthickness=2,bg='#c3c3c3')

    home_btn=tk.Button(options_fm,text='Home',
                       font=('Bold',15),fg=bg_color,bg='#c3c3c3',bd=0,command=lambda:switch(indecator=home_btn_indecator,page=home_page))
    home_btn.place(x=10,y=50)
    home_btn_indecator=tk.Label(options_fm,bg=bg_color)
    home_btn_indecator.place(x=5,y=48,width=3,height=40)

    student_card_btn=tk.Button(options_fm,text='Student\nCard',
                       font=('Bold',15),fg=bg_color,bg='#c3c3c3'
                       ,bd=0,justify=tk.LEFT,
                       command=lambda:switch(indecator=student_card_btn_indecator,page=dashboard_student_card_page))
    student_card_btn.place(x=10,y=100)
    student_card_btn_indecator=tk.Label(options_fm,bg='#c3c3c3')
    student_card_btn_indecator.place(x=5,y=108,width=3,height=40)
    
    security_btn=tk.Button(options_fm,text='security',
                       font=('Bold',15),fg=bg_color,bg='#c3c3c3'
                       ,bd=0,command=lambda:switch(indecator=security_btn_indecator,
                                                   page=security_page))
    security_btn.place(x=10,y=170)
    security_btn_indecator=tk.Label(options_fm,bg='#c3c3c3')
    security_btn_indecator.place(x=5,y=170,width=3,height=40)
    
    edit_data_btn=tk.Button(options_fm,text='Edit Data',
                       font=('Bold',15),fg=bg_color,bg='#c3c3c3'
                       ,bd=0,command=lambda:switch(indecator=edit_data_btn_indecator,
                                                   page=edit_data_page))
    edit_data_btn.place(x=10,y=220)
    edit_data_btn_indecator=tk.Label(options_fm,bg='#c3c3c3')
    edit_data_btn_indecator.place(x=5,y=220,width=3,height=40)
    

    delete_data_btn=tk.Button(options_fm,text='Delete\nAccount',
                       font=('Bold',15),fg=bg_color,bg='#c3c3c3'
                       ,bd=0,justify=tk.LEFT,command=lambda:switch(indecator=delete_btn_indecator
                                                                   ,page=delete_account_page))
    delete_data_btn.place(x=10,y=270)
    delete_btn_indecator=tk.Label(options_fm,bg='#c3c3c3')
    delete_btn_indecator.place(x=5,y=280,width=3,height=40)
    ...
    def home_page():
        ...
        student_img=ImageTk.PhotoImage(output)

        home_page_fm=tk.Frame(pages_fm)
        student_pic_lib=tk.Label(home_page_fm,image=student_img)
        student_pic_lib.image=student_img
        student_pic_lib.place(x=10,y=10)
...
"""
        student_card_img_obj=student_card(student_pic_path=student_pic
        ...
        dashboard_student_card_fm=tk.Frame(pages_fm)

        card_lb=tk.Label(dashboard_student_card_fm,image=student_card_img)
        card_lb.image=student_card_img
        card_lb.place(x=20,y=50)
        save_student_card_btn = tk.Button(dashboard_student_card_fm, text="Save Student Card",
                                  font=("Bold", 15),bd=1, fg="white", bg=bg_color,command=save_student_card)
        save_student_card_btn.place(x=40,y=400)
        print_student_card_btn = tk.Button(dashboard_student_card_fm, text="🖨️",
                                  font=("Bold", 15),bd=1 ,fg="white", bg=bg_color,command=print_student_card)
        print_student_card_btn.place(x=240,y=400)
        dashboard_student_card_fm.pack(fill=tk.BOTH,expand=True)
    def security_page():
        def show_hide_password():
            if current_password_ent['show'] == '*':
                current_password_ent.config(show='')
                show_hide_btn.config(image=un_locked_icon)
            else:
                current_password_ent.config(show='*')
        ...
        student_current_password=fetch_student(f"SELECT password FROM data WHERE id_number == '{student_id}'")
    ...
    def edit_data_page():
        ...
        def check_invalid_email(email):
            pattern = "^[a-zA-Z0-9-]+([._a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+([._a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,4})$"
        ...
        def upl_pic():
            ...
            if path:
                img = ImageTk.PhotoImage(Image.open(path).resize((100, 100), Image.Resampling.LANCZOS))
                pic_path.set(path)
        ...
        def Check_inputs():
            ...
            if student_name_ent.get() == '':
            ...
            elif student_age_et.get() == '':
                student_age_et.config(highlightcolor='red'
                                        ,highlightbackground='red')
                student_age_et.focus()
                message_box(message='Student Full Age Is Required')
            elif student_contact_et.get() == '':
                student_contact_et.config(highlightcolor='red'
                                        ,highlightbackground='red')
                ...
                if pic_path.get() != '':
                    ...
                    cursor.execute("UPDATE data SET image=? WHERE id_number = ?", 
                            (new_picture_binary, f'{student_id}'))

                    connection.commit()
                    connection.close()
                
                name=student_name_ent.get()
                age=student_age_et.get()
                select_class=select_class_btn.get()
                contact_number=student_contact_et.get()
                email_address=student_email_ent.get()
                connection = sqlite3.connect('students_accounts.db')
                cursor = connection.cursor()
                cursor.execute(f"""
                UPDATE data SET name='{name}', age='{age}', class='{select_class}', 
        ...
        student_age_et = tk.Entry(edit_data_page_fm, font=('Bold', 15),
                            highlightcolor=bg_color, highlightbackground='gray',
                            highlightthickness=2
                            )
        student_age_et.bind('<KeyRelease>',
                          lambda e:remove_highlight_warning(entry=student_age_et))

        student_age_et.place(x=5, y=235, width=180)
        student_age_et.insert(tk.END,get_student_details[0][1])
        student_contact_lb = tk.Label(edit_data_page_fm, text='Contact Phone Number. ',
                            font=('Bold', 12))
        student_contact_lb.place(x=5, y=285)

        student_contact_et = tk.Entry(edit_data_page_fm, font=('Bold', 15),
                                highlightcolor=bg_color, highlightbackground='gray',
                                highlightthickness=2)
        student_contact_et.place(x=5, y=310, width=180)

        student_contact_et.bind('<KeyRelease>',
                            lambda e:remove_highlight_warning(entry=student_contact_et))
    ...
    def delete_account_page():
        def confirm_delete_account():
            confirmation=confirmation_box(message="⚠️ Do You Want to Delete\nYour Account?")
            if confirmation:
                connection = sqlite3.connect('students_accounts.db')
                cursor = connection.cursor()
                cursor.execute(f"""
    ...
    pages_fm=tk.Frame(dashboard_frame)
...
def admin_dashboard():
    ...
    def switch(indecator,page):
        home_btn_indecator.config(bg='#c3c3c3')
        find_student_btn_indecator.config(bg='#c3c3c3')
        announcement_btn_indecator.config(bg='#c3c3c3')
        indecator.config(bg=bg_color)
        for child in pages_fm.winfo_children():
            child.destroy()
            root.update()
    ...
    options_fm = tk.Frame(dashboard_fm,highlightbackground=bg_color,
                        highlightthickness=2,bg='#c3c3c3')
    home_btn=tk.Button(options_fm,text='Home',font=('Bold',15),fg=bg_color
                       ,bg='#c3c3c3',bd=0,command=lambda:switch(indecator=home_btn_indecator,page=home_page))
    home_btn.place(x=10,y=50)
    home_btn_indecator=tk.Label(options_fm,text='',bg=bg_color)
    home_btn_indecator.place(x=5,y=48,width=3,height=40)

    find_student_btn=tk.Button(options_fm,text='Find\nStudent',font=('Bold',15),fg=bg_color
                       ,bg='#c3c3c3',bd=0,justify=tk.LEFT,
                       command=lambda:switch(indecator=find_student_btn_indecator,page=find_student_page))
    find_student_btn.place(x=10,y=100)
    find_student_btn_indecator=tk.Label(options_fm,text='',bg='#c3c3c3')
    find_student_btn_indecator.place(x=5,y=108,width=3,height=40)
    
    announcement_btn=tk.Button(options_fm,text='Announce\n-Ment🔊',font=('Bold',15),fg=bg_color
                       ,bg='#c3c3c3',bd=0,justify=tk.LEFT,
                       command=lambda:switch(indecator=announcement_btn_indecator,page=announcement_page))
    announcement_btn.place(x=10,y=170)
    announcement_btn_indecator=tk.Label(options_fm,text='',bg='#c3c3c3')
    announcement_btn_indecator.place(x=5,y=180,width=3,height=40)
    ...
    def logout_function():
        ...
        if confirm:
            ...
            welcomePage()
            root.update()
    logout_btn=tk.Button(options_fm,text='Logout',font=('Bold',15),fg=bg_color
                       ,bg='#c3c3c3',bd=0,command=logout_function)
    ...
    def home_page():
        ...
        hi_lb=tk.Label(home_page_fm,text='!Hi Admin',
                       font=('Bold',15))
        ...
        students_number_lb=tk.Label(home_page_fm,text='',font=('Bold',13),
                                    justify=tk.LEFT)
        students_number_lb.place(x=20,y=170)
        for i in class_list:
            result = fetch_student(query=f"SELECT COUNT(*) FROM data WHERE class == '{i}' ")
    ...
    def find_student_page():
            def find_student():
                ...
                if find_by_option_btn.get()=='id':
                   found_data= fetch_student(query=f"""
...
"""
                student_card_img_obj=student_card(student_pic_path=student_pic
                                        ,student_data=student_details)
                Student_Card_Page(student_card_obj=student_card_img_obj
            ...
            def clear_results():
                ...
                for item in record_table.get_children():
                    record_table.delete(item)
                generated_student_card_btn.config(state=tk.DISABLED)
            
            search_filters=['id','name','class','gender']
            find_stuedent_page_fm=tk.Frame(pages_fm)
            find_student_recourd_lb=tk.Label(find_stuedent_page_fm,
                                                text='Find Student Record',font=('Bold',13),
                                                fg='white',bg=bg_color)
            find_student_recourd_lb.place(x=20,y=10,width=300)

            find_by_lb = tk.Label(find_stuedent_page_fm, text="Find By:", font=("Bold", 12))
            find_by_lb.place(x=15, y=50)
            find_by_option_btn=Combobox(find_stuedent_page_fm,font=('Bold',12),
                                        state='readonly',values=search_filters)
            find_by_option_btn.place(x=80,y=50,width=80)
            find_by_option_btn.set('id')
            search_input=tk.Entry(find_stuedent_page_fm,font=('Bold',12)
                                  )
            search_input.place(x=20,y=90)
            search_input.bind('<KeyRelease>',lambda e:find_student())

            record_table_lb=tk.Label(find_stuedent_page_fm,text='Record Table'
                                     ,font=('Bold',13),bg=bg_color,fg='white')
            record_table_lb.place(x=20,y=160,width=300)
            record_table=Treeview(find_stuedent_page_fm)
            record_table.place(x=0,y=200,width=350)
            record_table.bind('<<TreeviewSelect>>',lambda e:generated_student_card_btn.config(state=tk.NORMAL))
            ###################################
            ...
            generated_student_card_btn=tk.Button(find_stuedent_page_fm,
                                                 text='Generate Student Card',font=('Bold',13),bg=bg_color
                                                 ,fg='white',state=tk.DISABLED,command=generate_student_card)
            generated_student_card_btn.place(x=160,y=450)
            clear_btn=tk.Button(find_stuedent_page_fm,text='Clear',font=('Bold',13),bg=bg_color,
                                fg='white',command=clear_results)
            clear_btn.place(x=10,y=450)

            find_stuedent_page_fm.pack(fill=tk.BOTH,expand=True)
    ...
    def announcement_page():
        ...
        def collected_emails():
            fetched_emails=[]
            for _class in selected_class:
                email=fetch_student(f"SELECT email FROM data WHERE class == '{_class}'")
                for email_address in email:
                    fetched_emails.append(*email_address)
            thread=threading.Thread(target=send_announcement_emails,args=[fetched_emails])
            thread.start()
            
        def send_announcement_emails(email_address):
            ...
            message=f"<h3 style='white-space: pre-warp;'>{announcement_message.get('0.1',tk.END)}</h3>"
            sending_count=0
            for email in email_address:
                sending_lb.config(text=f"Sending To:\n{email}\n\n{sending_count}/{len(email_address)}")
        ...
        classess_lis_lb=tk.Label(announcement_page_fm,text='Select Classes to Announce',
                                 font=('Bold',12))
        classess_lis_lb.place(x=10,y=320)
        y_pos=350
...
def add_acount_page():
    ...
    def generate_id_number():
        ...
        if not check_id_already_exists(id_number=generated_id):
            student_id_id.config(state=tk.NORMAL)
            student_id_id.delete(0,tk.END)
            student_id_id.insert(tk.END,generated_id)
            student_id_id.config(state='readonly')
        else:
            generate_id_number()
    ...
    def check_input_validation():
        if student_name_ent.get() == '':
            ...
            student_age_et.config(highlightbackground='red',highlightcolor='red')
            student_age_et.focus()
            message_box(message='Enter Student Age is Required')
        elif student_contact_et.get()=='':
            student_contact_et.config(highlightbackground='red',highlightcolor='red')
            student_contact_et.focus()
            message_box(message='Student Contact Num is Required')
            ...
            if pic_path.get() != '':
                resize_pic = Image.open(pic_path.get()).resize((100, 100), Image.Resampling.LANCZOS)
                resize_pic.save('temp_pic.png')

                with open('temp_pic.png', 'rb') as read_data:
                    pic_data = read_data.read()
            else:
                with open('C:/the rood/Students Management&Registration Project/add_image.png', 'rb') as read_data:
                    pic_data = read_data.read()
                    pic_path.set('C:/the rood/Students Management&Registration Project/add_image.png')

            add_data(id_number=student_id_id.get(),
                     password=account_password_ent.get(),
                     name=student_name_ent.get(),
                     age=student_age_et.get(),
                     gender=student_gender.get(),
                     phone_number=student_contact_et.get(),
                     studentclass=select_class_btn.get(),
...
"""
            ...
            get_student_card=student_card(student_pic_path=pic_path.get()
                         ,student_data=data)
            Student_Card_Page(student_card_obj=get_student_card)
            add_account_page_fm.destroy()
            root.update()
            message_box(message='Account Successfully Created')
    ...
    student_age_et = tk.Entry(add_account_page_fm, font=('Bold', 15),
                            highlightcolor=bg_color, highlightbackground='gray',
                            highlightthickness=2
                            )
    student_age_et.bind('<KeyRelease>',
    ...
    student_age_et.place(x=5, y=305, width=180)
    ...
    student_id_id = tk.Entry(add_account_page_fm, font=('Bold', 18),bd=0)
    student_id_id.place(x=380, y=35, width=80)

    student_id_id.config(state='readonly')

    generate_id_number()

    id_info_1b = tk.Label(add_account_page_fm, text="""Automatically Generated ID Number
! Remember Using This ID Number
Student Will Login Account""", justify=tk.LEFT)
    id_info_1b.place(x=240, y=65)
